...
class GestionEventosControllerTest {
    ...
    void deberiaEliminarEventoYResponderConJson() throws Exception {
        ...
        JsonNode eventoCreado = objectMapper.readTree(
                crearRespuesta.getResponse().getContentAsString());
        long idCreado = eventoCreado.get("id").asLong();

        mockMvc.perform(
                        delete("/gestioneventos/eliminar/{id}", idCreado)
    ...
    void deberiaActualizarEventoYResponderConJson() throws Exception {
        ...
        mockMvc.perform(
                        put("/gestioneventos/actualizar/{id}", idCreado)
                                .contentType(MediaType.APPLICATION_JSON)
